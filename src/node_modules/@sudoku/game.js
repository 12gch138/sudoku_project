...
import { sencodeParser, UrlParser } from './url';
...
export function startCustom(sencode) {
	...
	let matrix = sencodeParser(sencode)
	grid.decodeSencode(grid.getSencode(matrix));
	cursor.reset();
	timer.reset();
	hints.reset();
}
...
/**
 * Start new game with a url
 *
 * @param {string} url
 */
export function startUrl(url) {
	difficulty.setCustom();
	let matrix = UrlParser(url)
	grid.decodeSencode(grid.getSencode(matrix));
	cursor.reset();
	timer.reset();
	hints.reset();
